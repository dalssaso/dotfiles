...
(defun hd/monaspace-radon-italics-and-comments ()
  ...
  (let ((radon "Monaspace Radon")
        (neon "Monaspace Neon"))
    ;; Ensure code-y fixed-pitch faces stay Neon
    (set-face-attribute 'fixed-pitch nil :family neon)

    ;; Make *all* italics use Radon
    (set-face-attribute 'italic nil :family radon :slant 'italic)

    ;; Comments (and doc comments) in Radon italics
...
;; Re-apply after Doom loads/changing themes (themes often reset faces)
...
(setq org-agenda-files (list "~/Dropbox/OrgNotes/Agenda.org"))
